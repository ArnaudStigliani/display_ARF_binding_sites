...
for (PWM in list(pwm_ARF5,pwm_ARF2))
{
    ...
    for (elt1 in sites)   
    {
        ...
        for (elt2 in sites)
        {
            ...
            {
                DR <- rbind(DR,c((elt2 - elt1), sites[i],scores_ARF5$pDOF34[i],scores_ARF5$pDOF34[j],str_sub(sequence,sites[i] +3, sites[i] +3 + 6 + (elt2 - elt1) -1)))
            }
        }
    }
    if(!is.null(DR))
    ...
    for (elt1 in sites_rev)   
    {
        ...
        for (elt2 in sites_rev)
        {
            ...
            if ((elt2 - elt1) < 20 && (elt2 - elt1) >0)
            {
                DR_rev <- rbind(DR_rev,c((elt2 - elt1), sites_rev[i],scores_ARF5_rev$pDOF34[i],scores_ARF5_rev$pDOF34[j],str_sub(sequence,sites_rev[i] +3, sites_rev[i] +3 + 6 + (elt2 - elt1) -1)))
            }
        }
    }
    if(!is.null(DR_rev))
    ...
    for (elt1 in sites)   
    {
        ...
        for (elt2 in sites_rev)
        {
            ...
            if ((elt2 - elt1) < 20 && (elt2 - elt1) >0)
            {
                ER <- rbind(ER,c((elt2 - elt1), sites[i],scores_ARF5$pDOF34[i],scores_ARF5_rev$pDOF34[j],str_sub(sequence,sites[i] +3, sites[i] +3 + 6 + (elt2 - elt1) -1)))
            }
        }
    }
    if(!is.null(ER))
    ...
    for (elt1 in sites_rev)   
    {
        ...
        for (elt2 in sites)
        {
            ...
            if ((elt2 - elt1) < 20 && (elt2 - elt1) >0)
            {
                IR <- rbind(IR,c((elt2 - elt1), sites_rev[i],scores_ARF5_rev$pDOF34[i],scores_ARF5$pDOF34[j],str_sub(sequence,sites_rev[i] +3, sites_rev[i] +3 + 6 + (elt2 - elt1) -1)))
            }
        }
    }
    if(!is.null(IR))
    ...
    if(!is.null(tab))
    {
        ...
        write.table(tab,paste("Interdistances_",ARF[k],"_DOF34",".csv",sep=""),col.names=NA,quote=FALSE,sep="\t")
    }
}
